...
//!    and the name go.
//!
//! Two more tests link a SECOND copy of the library into the program, beside
//! the shared object, and check that the copy that publishes second refuses.
//!
...
/// The example with the library's source linked into the executable AND a
/// shared object of the same library kept as a dependency: two copies in one
/// process. Where the dynamic linker binds both copies' references to one
/// record (the default), they share it; with `symbolic` the shared object is
/// linked `-Bsymbolic`, binds to its own, and each copy keeps a record of its
/// own. The fixture's record is the executable's; the shared object's is
/// `<library_dir>/libtask_context.so`.
fn build_two_copies(symbolic: bool) -> Option<Fixture> {
    if !have_a_compiler() {
        return None;
    }
    let dir = fresh_dir(if symbolic {
        "two_symbolic"
    } else {
        "two_shared"
    });
    ...
    let example = crate_dir().join("examples/tcx_example.c");
    let library = dir.join("libtask_context.so");
    let program = dir.join("tcx_example_two_copies");

    let mut flags = vec!["-fPIC", "-shared", "-Wl,-z,nodelete"];
    if symbolic {
        flags.push("-Wl,-Bsymbolic");
    }
    compile(&flags, &[&source], &library).expect("the shared object builds");
    // --no-as-needed: the executable defines every symbol it uses, so the
    // linker would otherwise leave the shared object out.
    let search = format!("-L{}", dir.display());
    let rpath = format!("-Wl,-rpath,{}", dir.display());
    compile(
        &[&search, "-Wl,--no-as-needed", "-ltask_context", &rpath],
        &[&example, &source],
        &program,
    )
    .expect("the example links with both copies");
    Some(Fixture {
        record_is_in: program.clone(),
        program,
        library_dir: Some(dir),
...
/// The record's run-time address and its bytes: from the ELF file and the
/// map to the record in memory. Nothing is asserted about its values.
fn read_the_record(fixture: &Fixture, pid: u32, memory: &Memory) -> (u64, Vec<u8>) {
...
/// Two copies that share one record (the default linkage): the copy that
/// publishes second - the executable's, since a library's constructors run
/// before the program's - refuses, and the program's first set says so.
#[test]
fn a_second_copy_that_shares_the_record_refuses() {
    let Some(fixture) = build_two_copies(false) else {
        return;
    };
    let mut command = Command::new(&fixture.program);
    command.env(
        "LD_LIBRARY_PATH",
        fixture.library_dir.as_ref().expect("a library directory"),
    );
    let output = command.output().expect("the example runs");
    assert!(
        !output.status.success(),
        "the example went on with two copies"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("TCX-ERROR set_task_context(request_id) rc=-9"),
        "stderr: {stderr}"
    );
}

/// Two copies that each keep a record: the copy that is first in the global
/// scope - the executable's - works, and the shared object's copy refuses and
/// leaves its own record unpublished.
#[test]
fn a_second_copy_with_a_record_of_its_own_refuses() {
    let Some(fixture) = build_two_copies(true) else {
        return;
    };
    let mut running = Running::start(&fixture);
    let first = running.phase(0);
    ...
    // The program's own copy is published and serves every thread.
    let info = find_the_record(&fixture, running.pid, &memory);
    for line in &first {
        check_thread(&memory, &info, line);
    }
    // The shared object's copy is mapped, has a record of its own, and
    // published nothing.
    let library = fixture
        .library_dir
        .as_ref()
        .unwrap()
        .join("libtask_context.so");
    let other = Fixture {
        record_is_in: library,
    ...
    assert_eq!(u32_at(&record, 12), 0, "recipe_generation");

    running.go_on();
    let _ = running.phase(1);
    running.go_on();
