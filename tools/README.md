...
This directory contains tools for analyzing Perfetto trace files generated by systing using SQL-based queries via `trace_processor_shell`.

## Prerequisites

Install `trace_processor_shell` from Perfetto:
...
```bash
trace_processor_shell trace.pb -q trace_queries.sql
```
...
Use `trace_processor_shell` interactively to run custom queries:
```bash
trace_processor_shell trace.pb
```
