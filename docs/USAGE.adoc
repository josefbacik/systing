...
The captured argument will show up as an annotation on the event in the trace, providing additional context when viewing in `perfetto`.
Each arg specifies an `arg_index` (0-based) and `arg_type` ("string" or "long").
