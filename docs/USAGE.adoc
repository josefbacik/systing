...
The captured argument will show up as a debug annotation on the event in the trace, providing additional context when viewing in `perfetto`.
Each arg specifies an `arg_index` (0-based), `arg_type` ("string" or "long"), and `arg_name` (the name of the debug annotation).
