...
Higher values give finer-grained profiles at the cost of more overhead and larger traces; very high rates may be throttled by the kernel's perf interrupt-rate limit and can increase ringbuf drops (raise `--ringbuf-size-mib` if needed). Ring buffers are sharded per CPU, up to 64 rings per family at a constant family byte budget; `--ringbuf-shards N` sets the count explicitly (`8` is the fixed layout of older releases), and kernels before 6.8 stay at 8 by default because inserting each ring costs them an RCU grace period at start.
