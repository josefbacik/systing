...
Currently the supported events are `usdt`, `uprobe`, `uretprobe`, `kprobe`, `kretprobe`, `tracepoint`, and `marker`.
...
Marker events provide a way for userspace applications to emit trace events without any special libraries or dependencies.
Applications emit markers by calling the `faccessat2` syscall with a specific match string:
...
// Emit a marker event
syscall(SYS_faccessat2, request_id, "myapp:request:start", F_OK, 0);
----

The first argument (`dfd`) can be used to pass additional context (like a request ID) that will be captured if configured in `args`.
...
For marker events, the `arg_index` refers to the `faccessat2` syscall arguments:

* `0` - `dfd` (directory file descriptor, can be used to pass context like request IDs)
* `1` - `filename` (the match string itself)
* `2` - `mode`
* `3` - `flags`

NOTE: The `retval` arg type is not supported for marker events since they are captured on syscall entry.
