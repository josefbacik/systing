...
/// left the event standing. `utime_delta_ns` / `stime_delta_ns` /
/// `runtime_delta_ns` are the thread's user, system and on-CPU time since its
/// last full record (0 on its first): its previous event, or in Python-only
/// mode a record that was left out for having no Python frames. The first two
/// advance by scheduler ticks, so 0 is not "no CPU"; the third is exact.
