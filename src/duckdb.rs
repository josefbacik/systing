...
pub fn create_schema(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS heap_snapshot (
            ...
            dumped_at_unix_ns BIGINT, -- the file's mtime
            sample_period BIGINT, -- mean bytes between samples, as the dump states it
            -- The dump's header totals, as written. Not always the sum of its
            -- stacks (jemalloc 5.3 interval dumps disagree); sum heap_sample
            -- for a snapshot's total.
            header_live_objects BIGINT,
            header_live_bytes BIGINT
        );

        -- One row per distinct allocation stack in a snapshot. Counts are the
        -- allocator's as written in the file (jemalloc >= 5.3 already
        -- un-biases them for sampling); alloc_* are cumulative since start
        -- and 0 unless the allocator tracked them (jemalloc prof_accum).
...
/// The stack INSERT deliberately avoids two DuckDB shapes that blow through
/// any `memory_limit` on large traces (a whole-system trace has ~1M unique
/// stacks; the observed failure is "Out of Memory Error: failed to allocate
/// data" mid-import, with spill to `temp_directory` unable to help):
///
/// - a lateral `FROM t, unnest(...) WITH ORDINALITY` compiles into DELIM_JOIN
///   machinery with materialized intermediates; `unnest` +
///   `generate_subscripts` in SELECT position streams through the plain
///   unnest operator instead;
/// - `list(x ORDER BY y)` feeding an INSERT exhausts the buffer pool even at
///   a fraction of the trace (the same aggregate under a plain SELECT is
///   fine). Aggregating unordered `struct_pack` pairs and ordering each
///   row's list afterwards (`list_sort` + `list_transform`, both row-local)
///   is semantically identical and stays within budget: the same fixture
///   that fails the ordered-aggregate INSERT at 8x the budget imports whole
///   this way.
pub fn import_stack_from_parquet(conn: &Connection, path: &Path, trace_id: &str) -> Result<()> {
    ...
    conn.execute_batch(&format!(
        "INSERT INTO frame \
         ...
         INSERT INTO stack \
           SELECT '{escaped_trace_id}', u.id, \
                  list_transform(list_sort(list(struct_pack(i := u.idx, f := fr.id))), \
                                 x -> x.f), \
                  u.depth, u.leaf_name \
           FROM (SELECT id, depth, leaf_name, \
                        unnest(frame_names) AS fname, \
                        generate_subscripts(frame_names, 1) AS idx \
                 FROM read_parquet('{escaped_path}')) u \
           JOIN frame fr ON fr.trace_id = '{escaped_trace_id}' AND fr.name = u.fname \
           GROUP BY u.id, u.depth, u.leaf_name \
           ORDER BY u.id;"
    ))
    .with_context(|| {
        format!(
...
mod tests {
    ...
    /// Regression test for the production failure mode: a large trace's
    /// stack/frame normalization must import under a bounded `memory_limit`.
    /// With the previous statement shape (lateral `WITH ORDINALITY` +
    /// `list(... ORDER BY ...)` feeding the INSERT), this exact fixture and
    /// budget fail with DuckDB's "Out of Memory Error: failed to allocate
    /// data ..." — spill to `temp_directory` cannot save it. See the
    /// `import_stack_from_parquet` doc comment for the two shapes involved.
    #[test]
    fn test_import_stack_under_memory_limit() {
        let tmp = TempDir::new().unwrap();
        let parquet_path = tmp.path().join("stack.parquet");

        // Synthesize a fat stack.parquet with a deterministic frame pattern:
        // 150K stacks, depth 40, over an 8K-name vocabulary. Generated by an
        // unconstrained side connection; the import connection is the capped
        // one. Small row groups keep the reader's per-chunk decompression
        // floor well under the test budget, so the budget discriminates the
        // normalization's memory behavior, not scan buffering.
        {
            let generator = Connection::open_in_memory().unwrap();
            generator
                .execute_batch(&format!(
                    "COPY ( \
                       SELECT i AS id, \
                              list_transform(range(40), \
                                             j -> 'frame_' || ((i * 7 + j * 13) % 8000)) \
                                AS frame_names, \
                              40 AS depth, \
                              'frame_' || ((i * 7) % 8000) AS leaf_name \
                       FROM range(150000) t(i) \
                     ) TO '{}' (FORMAT PARQUET, ROW_GROUP_SIZE 20000)",
        ...
        // A budget the old statement shape exhausts on this fixture (it
        // failed even given 1/16th of the rows), with room to spare for the
        // streaming shape plus the scan floor.
        conn.execute_batch(&format!(
        ...
        import_stack_from_parquet(&conn, &parquet_path, "memtest").unwrap();

        // Frame vocabulary interned exactly once, ids dense and unique.
        let frames: i64 = conn
            .query_row(
                "SELECT count(*) FROM frame WHERE trace_id = 'memtest'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(frames, 8000);
        let dup_names: i64 = conn
            .query_row(
                "SELECT count(*) FROM (SELECT name FROM frame WHERE trace_id = 'memtest' \
                 GROUP BY name HAVING count(*) > 1)",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(dup_names, 0, "frame names must be interned exactly once");
        let (min_id, max_id, distinct_ids): (i64, i64, i64) = conn
            .query_row(
                "SELECT min(id), max(id), count(DISTINCT id) FROM frame \
                 WHERE trace_id = 'memtest'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
            )
            .unwrap();
        assert_eq!((min_id, max_id, distinct_ids), (0, 7999, 8000));

        // Every stack imported exactly once.
        let (stacks, distinct_stacks): (i64, i64) = conn
            .query_row(
                "SELECT count(*), count(DISTINCT id) FROM stack WHERE trace_id = 'memtest'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
        assert_eq!((stacks, distinct_stacks), (150000, 150000));

        // Spot-check one stack round-trips through the stack_frames view in
        // frame order.
        let names: String = conn
            .query_row(
                "SELECT array_to_string(frame_names, ',') FROM stack_frames \
                 WHERE trace_id = 'memtest' AND id = 4242",
                [],
                |r| r.get(0),
            )
            .unwrap();
        let expected: Vec<String> = (0..40)
            .map(|j| format!("frame_{}", (4242 * 7 + j * 13) % 8000))
            .collect();
