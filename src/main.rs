...
struct Command {
    ...
    /// The form of the memory recorder's mmap/munmap/brk hooks and the network recorder's TIME_WAIT hooks: `classic` (the default: the six syscalls tracepoints through perf_event_open, and kprobes), `raw-tracepoint` (for the memory hooks one raw tp_btf program each on the sys_enter and sys_exit tracepoints dispatching on the syscall number, with the classic set as the fallback on a kernel whose BTF lacks the sys_enter/sys_exit typedefs; the network hooks stay kprobes) or `trampoline` (fentry/fexit programs on the kernel functions, with the classic set as the attach-time fallback). Each opt-in form trades one cost for another: the raw tracepoint pair detaches asynchronously instead of waiting six synchronous grace periods on the capture's stop path, but it runs two BPF programs on every syscall of every task while attached (a getpid() storm ran at about half of its uncaptured rate under the pair against 84-88% under the classic set on a 4-vCPU test guest, three runs); the trampoline form puts nothing on other syscalls' entry path while the capture runs, but its detach runs two synchronous Tasks-RCU grace periods per hooked function on the stop path — about six times the classic set's `detach bpf programs` phase. Which form ran is `sysinfo.memory_syscall_leg` / `sysinfo.network_tw_leg`.
    #[arg(long, default_value = "classic", value_parser = clap::builder::PossibleValuesParser::new(["classic", "raw-tracepoint", "trampoline"]))]
    kernel_hooks: String,
    ...
    #[arg(skip)]
    task_stacks: bool,
    /// With the task-stacks recorder, the frames to collect: `native` (kernel and native user frames), `python` (Python frames alone, and only the threads that have any) or `all` (both). `python` and `all` turn --collect-pystacks on. Not given: `all` with --collect-pystacks, else `native`
    #[arg(long, value_parser = clap::builder::PossibleValuesParser::new(["native", "python", "all"]))]
    task_stacks_frames: Option<String>,
...
impl From<Command> for Config {
    fn from(cmd: Command) -> Self {
        Config {
            ...
            task_stacks_frames: cmd.task_stacks_frames.map(|frames| {
                frames
                    .parse()
                    .expect("clap admits only the values TaskStackFrames parses")
            }),
...
fn main() -> Result<()> {
    ...
    // The task-stacks recorder keeps its events until the end, so a capture
    // that runs for good would grow for good.
    if opts.task_stacks && opts.continuous > 0 {
        anyhow::bail!(
            "the task-stacks recorder cannot be used with --continuous: it keeps every \
             event until the capture ends"
        );
    }

    // --task-stacks-frames is the task-stacks recorder's; asking for Python
    // frames needs the Python stack walker.
    if let Some(frames) = &opts.task_stacks_frames {
        if !opts.task_stacks {
            anyhow::bail!(
                "--task-stacks-frames applies to the task-stacks recorder (--add-recorder task-stacks)"
            );
        }
        if frames != "native" {
...
mod tests {
    ...
    fn task_stacks_is_off_by_default_and_selectable() {
        ...
        assert_eq!(opts.task_stacks_interval_ms, 100);
    ...
    #[test]
    fn task_stacks_frames_takes_the_three_names_and_nothing_else() {
        for frames in ["native", "python", "all"] {
            let opts = opts_from(&[
                "--add-recorder",
                "task-stacks",
                "--task-stacks-frames",
            ...
            assert_eq!(opts.task_stacks_frames.as_deref(), Some(frames));
        }
        assert_eq!(opts_from(&[]).task_stacks_frames, None);
        assert!(Command::try_parse_from(["systing", "--task-stacks-frames", "system"]).is_err());
        assert!(Command::try_parse_from(["systing", "--only-pystacks"]).is_err());
    }
