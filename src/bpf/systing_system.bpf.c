...
/* Every filter except the --pid / --cgroup targeting; see trace_task() and
 * trace_task_btf(). */
...
{
	if (!trace_task_common(task) || !task_in_target_pids(task))
		return false;
...
{
	return trace_task_common(task) && task_in_target_set(task);
}
