...
pub use sched_aggregate::{
    ...
    SwitchStats, TailContributor, TruncationReason, DEFAULT_CHUNK_ROWS, DEFAULT_MAX_ROWS,
    HIST_BASE_LOG2, HIST_OCTAVES,
};
pub use sched_stats::{
...
pub struct TraceSystemInfo {
    ...
    /// ("tracepoint" — the classic set, the default form; "raw_tracepoint" —
    /// the opt-in raw-tracepoint form from the next release (unreleased at
    /// the time of writing): one tp_btf program each on sys_enter and
    /// sys_exit; "tracepoint:nobtf", "tracepoint:noraw" — the classic set
    /// under that form, with why the raw pair did not run; "fentry" — the
    /// opt-in trampoline set, the default in 1.17.0–1.17.2;
    /// "tracepoint:nosym", "tracepoint:nobtf", "tracepoint:notramp" — the
    /// classic set under the trampoline form, with why; or "off:<cause>" —
    /// the capture has no mmap/munmap/brk rows in memory_map); `None` when
    /// the memory recorder did not run or for traces from systing < 1.17,
    /// which always attached the classic tracepoints.
    #[serde(skip_serializing_if = "Option::is_none")]
